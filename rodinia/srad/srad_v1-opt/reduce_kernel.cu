...
__global__ void reduce(	long d_Ne,											// number of elements in array
	...
	// reduction of sums if last block is not full (common case)
		...
		if(bx != (gridDim.x - 1)){											//
			// sum of every 2, 4, ..., NUMBER_THREADS elements
			for(i=2; i<NUMBER_THREADS/4; i=2*i){								//
				// sum of elements
				if((tx+1) % i == 0){										// every ith
					d_psum[tx] = d_psum[tx] + d_psum[tx-i/2];
					d_psum2[tx] = d_psum2[tx] + d_psum2[tx-i/2];
			...
			// final sumation by last thread in every block
			if(tx==(NUMBER_THREADS-1)){										// block result stored in global memory
				d_sums[bx*d_mul*NUMBER_THREADS] = d_psum[NUMBER_THREADS / 8 - 1] + d_psum[NUMBER_THREADS / 8 * 2 - 1] + d_psum[NUMBER_THREADS / 8 * 3 - 1] + d_psum[NUMBER_THREADS / 8 * 4- 1] + d_psum[NUMBER_THREADS / 8 * 5 - 1] + d_psum[NUMBER_THREADS / 8 * 6 - 1] + d_psum[NUMBER_THREADS / 8 * 7 - 1] + d_psum[NUMBER_THREADS - 1];
				d_sums2[bx*d_mul*NUMBER_THREADS] = d_psum2[NUMBER_THREADS / 8 - 1] + d_psum2[NUMBER_THREADS / 8 * 2 - 1] + d_psum2[NUMBER_THREADS / 8 * 3 - 1] + d_psum2[NUMBER_THREADS / 8 * 4- 1] + d_psum2[NUMBER_THREADS / 8 * 5 - 1] + d_psum2[NUMBER_THREADS / 8 * 6 - 1] + d_psum2[NUMBER_THREADS / 8 * 7 - 1] + d_psum2[NUMBER_THREADS - 1]; 
			}
		}
