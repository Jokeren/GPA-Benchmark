...
cudaFuncSetAttribute(tensor_transpose, cudaFuncAttributePreferredSharedMemoryCarveout, 100);
cudaFuncSetAttribute(tensor_transpose, cudaFuncAttributeMaxDynamicSharedMemorySize, 2 * 4700 * sizeof(double));
...
#if defined CUDA4 || defined CUDA5
tensor_transpose<<<nblocks, NTHREADS, 2 * (4700) * sizeof(double)>>>(dim_input, dim_output, nblocks, tile_size,
